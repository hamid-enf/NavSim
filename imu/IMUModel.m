...
% optional bias random walk. All error sources are toggled live from config.
%
...
methods
    function updateParams(obj, cfg)
        obj.cfg = cfg;
        if cfg.IMU.useGyroBias, obj.bgBase = deg2rad(cfg.IMU.gyroBiasDps(:));
        else, obj.bgBase = zeros(3,1); end
        if cfg.IMU.useAccelBias, obj.baBase = cfg.IMU.accelBiasMg(:) * 1e-3 * 9.80665;
        else, obj.baBase = zeros(3,1); end
