...
methods
    ...
    function update(obj, snap)
        ...
        v{end+1} = sprintf('   pos NIS %.2f / gate %.2f  accepted=%d', ...
            snap.pred.rawNIS, snap.pred.gate, snap.pred.posAccepted);
        v{end+1} = sprintf('   sample accepted=%d  vel accepted=%d', ...
            snap.pred.accepted, snap.pred.velAccepted);
        v{end+1} = sprintf('   NIS reject=%d  OOSM applied/reject/old=%d/%d/%d', ...
            snap.pred.rejected, snap.pred.oosmApplied, snap.pred.oosmRejected, snap.pred.oosmTooOld);
        v{end+1} = ' ';
